...
class News extends Admin_Controller
{
	...
	public function index()
	{
		...
		$this->data['newscount'] = $this->getbase->get_count('news_content');
		$this->data['news'] = $this->getbase->getjoin('news_content', 'menu', 'menuid', 'id', 0, 50, '0', '0', 'id', 'desc', 'menuname');


		$this->render('apanel/news/home');
	...
	public function save()
	{
		...
		if ($ids == "") {
			$inserted = $this->getbase->insert('news_content', $newssave);
			...
			$this->getbase->update('news_content', $insert, $insertid);
		} else {
			$this->getbase->update('news_content', $newssave, $ids);
			...
			$this->getbase->update('news_content', $update, $ids);
		}
	...
	{
		// if ($this->session->userdata("login")) {
		$this->data["news"] = $this->getbase->getinfo('news_content', $ids);
		$this->data["menulist"] = $this->getbase->getinfo('menu', -1, 1, 100, "language", $this->current_lang, "sort");
		// $this->load->view("apanel/top", $data);
	...
	{
		// if ($this->session->userdata("login")) {
		$this->getbase->delete('news_content', $ids);
		redirect(base_url('admin/news'), 'refresh');
		// } else {
		// 	redirect(base_url('apanel/login'), 'refresh');
		// }
