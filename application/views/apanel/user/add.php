	...
	<div id="body" class="ml-64 mt-20">
		...
		<table cellpadding="5" border="0" align="left" class="table-border-none">
			...
			<tr class='tablebody'>
				...
				<td><?php if(isset($error1)) {echo "<div style='color:red;font-weight:bold;'>".$error1."</div>";} ?></td>
			</tr>
			<tr class='tablebody' >
			...
			<tr class='tablebody' >
				...
				<td>
					<select name="level">
						<option value="0" <?php if(isset($user[0]->level)) {if($user[0]->level==0) {echo "selected";}}?>>User</option>
						<option value="1" <?php if(isset($user[0]->level)) {if($user[0]->level==1) {echo "selected";}}?>>Admin</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td><button type="submit">Хадгалах</button>
					<a href="<?=base_url("apanel/user");?>">Болих</a></td>
			</tr>
		</table>
		</form>
	</div>
